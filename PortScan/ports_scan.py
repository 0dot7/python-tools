import socket, threading, queue, logging, argparse

from scapy.layers.inet import ICMP, IP, UDP, TCP
from scapy.sendrecv import sr1, send

logging.basicConfig(filename='./ports_scan.log', level=logging.INFO,
...
def connect_scan(ip, port):
    try:
        ...
        if result == 0:
            ...
            logging.info("%s port %s is down" % (ip, port))
            sock.close()

    except Exception as e:
        logging.exception("Exception    while connecting    to %s: %s" % (ip, e))
        print(e)
...
def udp_scan(ip, port):
    try:
        ans = sr1(IP(dst=ip) / UDP(dport=port), timeout=1, verbose=False)
        if ans is not None:
            if ans.haslayer(ICMP):
                icmp_fields = ans.getlayer(ICMP).fields
                if icmp_fields["type"] == 3 and icmp_fields["code"] == 3:
                    logging.info("%s port %s is closed" % (ip, port))
                else:
                    logging.info("%s port %s is filtered" % (ip, port))
            if ans.haslayer(UDP):
                logging.info("%s port %s is open" % (ip, port))
                print("%s port %s is open" % (ip, port))
        elif ans is None:
            logging.info("%s port %s is no response" % (ip, port))
    except Exception as e:
        logging.exception("Exception    while sending " + str(ip) + " to " + str(port))
        print(e)
...
def syn_scan(ip, port):
    try:
        ...
        logging.exception("Exception    while   trying to connect to    %s: %s" % (ip, e))
        print(e)
...
def fin_scan(ip, port):
    try:
        ans = sr1(IP(dst=ip) / TCP(dport=port, flags="F"), timeout=1, verbose=False)
        if ans is None:
            logging.info("%s port %s is open | filtered" % (ip, port))
...
def null_scan(ip, port):
    try:
        ans = sr1(IP(dst=ip) / TCP(dport=port, flags=""), timeout=1, verbose=False)
        if ans is None:
            logging.info("%s port %s is open | filtered" % (ip, port))
            print("%s port %s is open | filtered" % (ip, port))
        elif ans is not None:
...
def ack_scan(ip, port):
    try:
        ...
        if ans is None:
            logging.info("%s port %s is filtered and FireWall is up" % (ip, port))
        elif ans is not None:
            if ans.haslayer(TCP) and ans.getlayer(TCP).flags == 'RA':
                logging.info("%s port %s is unfiltered" % (ip, port))
                print("%s port %s is unfiltered and FireWall is down" % (ip, port))
            elif ans.haslayer(ICMP) and ans.getlayer(ICMP).type == 3 and ans.getlayer(ICMP).code in [1, 2, 3, 9, 10,
                                                                                                     13]:
                logging.info("%s port %s is filtered and FireWall is up" % (ip, port))
    except Exception as e:
        logging.exception("Exception")
...
def window_scan(ip, port):
    try:
        ans = sr1(IP(dst=ip) / TCP(dport=port, flags="A"), timeout=1, verbose=False)
        if ans is None:
            logging.info("%s port %s is closed | filtered" % (ip, port))
        ...
        logging.exception("Exception    while connecting to %s  on      port %s" % (ip, port))
        print(e)


# 主函数-多线程扫描
def mult_scan(scan_type, ip, portlist):
    try:
        logging.info("------ %s Scan Starting ------" % str(ip))
        threads = []
        q = queue.Queue()
        for port in portlist:
            threads.append(threading.Thread(target=scan_type, args=(str(ip), int(port))))
        for t in threads:
            q.put(t)
        while not q.empty():
            q.get().start()
        for t in threads:
            t.join()
        logging.info("------ %s Scan Finished ------" % str(ip))
    except Exception as e:
        logging.exception("Exception during scan: " + str(e))
        print(e)


masscan_top_tcp_ports = {1, 3, 4, 6, 7, 9, 13, 17, 19, 20, 21, 22, 23, 24, 25, 26, 30, 32, 33, 37,
                         42, 43, 49, 53, 70, 79, 80, 81, 82, 83, 84, 85, 88, 89, 90, 99, 100, 106,
                         109, 110, 111, 113, 119, 125, 135, 139, 143, 144, 146, 161, 163, 179,
                         199, 211, 212, 222, 254, 255, 256, 259, 264, 280, 301, 306, 311, 340,
                         366, 389, 406, 407, 416, 417, 425, 427, 443, 444, 445, 458, 464, 465,
                         481, 497, 500, 512, 513, 514, 515, 524, 541, 543, 544, 545, 548, 554,
                         555, 563, 587, 593, 616, 617, 625, 631, 636, 646, 648, 666, 667, 668,
                         683, 687, 691, 700, 705, 711, 714, 720, 722, 726, 749, 765, 777, 783,
                         787, 800, 801, 808, 843, 873, 880, 888, 898, 900, 901, 902, 903, 911,
                         912, 981, 987, 990, 992, 993, 995, 999, 1000, 1001, 1002, 1007, 1009,
...
simple_ports = {21, 22, 23, 25, 53, 69, 80, 81, 82, 83, 84, 85, 86,
                87, 88, 89, 110, 135, 139, 143, 161, 443, 445, 465,
                873, 993, 995, 1080, 1158, 1433, 1521, 1723, 1863,
                2049, 2100, 3128, 3306, 3389, 4100, 5000, 5432, 5632,
                5900, 6379, 7001, 8069, 8080, 8081, 8082, 8083, 8084,
                8085, 8086, 8087, 8088, 8089, 8090, 9090, 9200, 9300,
                9871, 11211, 27017, 27018}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-sT', '--connect', action='store', dest='connect_ip', nargs='?', type=str,
                        help='TCP connect() scan')
    parser.add_argument('-sU', '--udp', dest='udp_ip', nargs='?', type=str, help='UDP scan')
    parser.add_argument('-sS', '--syn', dest='syn_ip', nargs='?', type=str, help='TCP SYN scan')
    parser.add_argument('-sF', '--fin', dest='fin_ip', nargs='?', type=str, help='FIN scan')
    parser.add_argument('-sN', '--null', dest='null_ip', nargs='?', type=str, help='Null scan')
    parser.add_argument('-sX', '--xmas', dest='xmas_ip', nargs='?', type=str, help='Xmas scan')
    parser.add_argument('-sA', '--ack', dest='ack_ip', nargs='?', type=str, help='TCP ACK scan')
    parser.add_argument('-sW', '--window', dest='window_ip', nargs='?', type=str, help='TCP Window scan')
    parser.add_argument('--all-ports', action='store_true', default=False, help='select all ports')
    ...
    if args.connect_ip:
        mult_scan(connect_scan, args.connect_ip, ports)
    elif args.udp_ip:
        mult_scan(udp_scan, args.udp_ip, ports)
    elif args.syn_ip:
        mult_scan(syn_scan, args.syn_ip, ports)
    elif args.fin_ip:
        mult_scan(fin_scan, args.fin_ip, ports)
    elif args.null_ip:
        mult_scan(null_scan, args.null_ip, ports)
    elif args.xmas_ip:
        mult_scan(xmas_scan, args.xmas_ip, ports)
    elif args.ack_ip:
        mult_scan(ack_scan, args.ack_ip, ports)
    elif args.window_ip:
        mult_scan(window_scan, args.window_ip, ports)
    else:
